# Testing only positve, negative, no neutral
...
# Compute accuracy statistics (all sentiments)
def compute_metrics(actual, predicted, model_name):
    ...
    print(f"\n{model_name} Performance Metrics (All Sentiments):")
    ...
    plt.title(f'{model_name} Confusion Matrix (All Sentiments)')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.savefig(f'P2_Data_Analysis/Sentimentdata/{model_name.lower().replace(" ", "_")}_confusion_matrix_all.png')
    plt.close()

# Compute metrics for both dictionaries (all sentiments)
...
# Compute accuracy statistics (positive and negative only)
def compute_metrics_binary(actual, predicted, model_name):
    accuracy = accuracy_score(actual, predicted)
    precision, recall, f1, _ = precision_recall_fscore_support(actual, predicted, average='weighted', zero_division=0)
    cm = confusion_matrix(actual, predicted, labels=['positive', 'negative'])
    
    print(f"\n{model_name} Performance Metrics (Positive/Negative Only):")
    print(f"Accuracy: {accuracy:.4f}")
    print(f"Precision: {precision:.4f}")
    print(f"Recall: {recall:.4f}")
    print(f"F1-Score: {f1:.4f}")
    ...
    plt.title(f'{model_name} Confusion Matrix (Positive/Negative Only)')
    plt.xlabel('Predicted')
    plt.ylabel('Actual')
    plt.savefig(f'P2_Data_Analysis/Sentimentdata/{model_name.lower().replace(" ", "_")}_confusion_matrix_binary.png')
    plt.close()

# Filter for positive and negative sentiments only
binary_df = df[df['actual_sentiment'].isin(['positive', 'negative'])]

# Compute metrics for both dictionaries (positive/negative only)
compute_metrics_binary(binary_df['actual_sentiment'], binary_df['Loughran_sentiment'], "Loughran-McDonald")
compute_metrics_binary(binary_df['actual_sentiment'], binary_df['AFINN_sentiment'], "AFINN")